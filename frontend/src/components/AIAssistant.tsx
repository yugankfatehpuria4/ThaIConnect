...
export default function AIAssistant() {
  ...
  const sendMessage = async () => {
    ...
    try {
      // Issue #18 fixed: call via backend proxy (/api/ai/chat) not localhost:5001 directly
    ...
    <div className="fixed bottom-6 right-6 z-50">
      {isOpen ? (
        <div className="w-80 sm:w-96 bg-white border border-gray-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all">
          <div className="bg-red-500 text-white px-4 py-3 flex justify-between items-center bg-red">
          ...
          <div className="p-3 bg-white border-t border-gray-100 flex gap-2">
            ...
            <button
              onClick={sendMessage}
              disabled={loading}
              className="px-4 py-2 bg-red text-white rounded-xl text-sm font-medium hover:bg-red/90 transition-colors disabled:opacity-50"
            >
              Send
            </button>
          </div>
